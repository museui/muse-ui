...
export default {
  description,
  simpleExample: '简单的使用',
  simpleExampleDesc: 'dateTimePicker 默认是以竖屏弹出框的是形式选择，有横屏和竖屏两种显示模式，通过 `mode=landscape` or `mode=portrait` 切换',
  inlineExample: '内联弹出模式',
  inlineExampleDesc: '设置 `container=inline` 可以使选择框以内联方式弹出',
  controlExample: '选择控制',
  controlExampleDesc: '和其它表单组件一样，dateTimePicker 也可以通过 `v-model` 控制value属性，`maxDate` 和 `minDate` 控制选择日期范围，设置 `autoOk` 和 `disableYearSelection` 来切换自动确定和关闭年份选择功能',
